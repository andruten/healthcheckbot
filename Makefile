...
run: check_env build
	@$(DOCKER) run --name healthcheckbot -u "$(id -u):$(id -g)" --restart on-failure:3 --env-file .env -v $(CURRENT_DIR):/opt/app healthcheckbot:latest

run_detached: check_env build
	@$(DOCKER) run -d --name healthcheckbot -u "$(id -u):$(id -g)" --restart on-failure:3 --env-file .env -v $(CURRENT_DIR):/opt/app -ti healthcheckbot:latest
